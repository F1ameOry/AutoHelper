...
import { useAuthStore, useCars2Store, useInvoiceStore } from '@/stores';
import router from '@/router';

const {demo} = useAuthStore()

const car = demo? useCars2Store().demoCars2.find( car => car.id === router.currentRoute.value.params.id ) : useCars2Store().cars2.find( car => car.id === router.currentRoute.value.params.id )
    ...
    <div>
      ...
      <div class="carView flex flex-col mt-12 gap-2">
        ...
        <div class="flex flex-col gap-2">
          <span class="text-xl text-white font-semibold">Volkswagen Caravelle</span>
          <span> Пробег 64 759 км</span>
          <div class="flex flex-row gap-2 items-center">
            <div class="h-2 w-2 rounded-full bg-[#FFAC0A]"></div>
              <span> Последнее ТО - 6 285 км назад (24.05.2024)</span>
          </div>
        </div>
      </div>
